...
public:
  ...
  constexpr Box( const Vec3<T>& a, const Vec3<T>& b )  noexcept 
    : m_({ a.x, b.x, a.y, b.y, a.z, b.z })
  { 
    if (b.x < a.x) { m_[0] = b.x; m_[1] = a.x; }
    if (b.y < a.y) { m_[2] = b.y; m_[3] = a.y; }
    if (b.z < a.z) { m_[4] = b.z; m_[5] = a.z; }
  }

  constexpr Box(T left, T bottom, T right, T top, T front, T back) noexcept
    : m_({ left, right, bottom, top, front, back }) {}

  template< class Y > Box( const Box<Y>& b ) 
    : m_({ (T)b.m_[0], (T)b.m_[1],(T)b.m_[2],(T)b.m_[3],(T)b.m_[4],(T)b.m_[5] }) {}
  
  constexpr bool is_empty() const noexcept
  { 
    return m_[0] == std::numeric_limits<T>::max(); // should rather check for reverse limits?
  }

  void set_empty() noexcept 
  { 
    m_[(int)Dim::Left] = std::numeric_limits<T>::max();
    m_[(int)Dim::Right] = std::numeric_limits<T>::lowest();
    m_[(int)Dim::Bottom] = std::numeric_limits<T>::max();
    m_[(int)Dim::Top] = std::numeric_limits<T>::lowest();
    m_[(int)Dim::Front] = std::numeric_limits<T>::max();
    m_[(int)Dim::Back] = std::numeric_limits<T>::lowest();
  }

  constexpr T width() const noexcept { return right() - left(); }
  constexpr T height() const noexcept { return top() - bottom(); }
  constexpr T depth() const noexcept { return back() - front(); }
  T get_extent(int axis) const noexcept // returns -std::numeric_limits<T>::infinity() for empty box or invalid axis
  {
    I3S_ASSERT(axis >= 0 && axis <= 2);
    if (axis < 0 || axis > 2)
    {
      I3S_ASSERT(false);
    ...
    if constexpr (std::numeric_limits<T>::has_infinity)
       return m_[2 * axis + 1] - m_[2 * axis];
    else
    {
      if (m_[2 * axis + 1] < m_[2 * axis])
      {
        I3S_ASSERT(false);
        return T(0);
      }
  ...
  void expand( const Box<T>& b ) noexcept;
  void scale_by( T factor ) noexcept;
  constexpr T operator[]( Dim d ) noexcept { return m_[static_cast<int>(d)]; }
  const T& operator[]( Dim d ) const noexcept { return m_[static_cast<int>(d)]; }
  //DEPRECATED (Contact Sergey Korytnik before uncommenting): Box<T>&   operator*(T s) noexcept { for (auto& v : m_) v *= s; return *this; }
  ...
  void      move_by(const Vec3<T>& d) noexcept 
  {
    I3S_ASSERT(!is_empty());
    left() += d.x; right() += d.x; bottom() += d.y; top() += d.y; front() += d.z; back() += d.z; 
  }
  ...
  Box<T>& operator*=(T s) noexcept 
  { 
    if (!is_empty()) { for (auto& x : m_) x *= s; } return *this; 
  }

  // DEPRECATED(contact Sergey Korytnik before uncommenting) Box<T>    slice2d( int x, int y, int w, int h ) const;

  constexpr bool _is_degenerated() const noexcept 
  { 
...
template< class T >
inline void Box<T>::expand( const Box<T>& b ) noexcept
{
...
template< class T >
void  Box<T>::scale_by( T v ) noexcept
{
  if( !is_empty() )
  {
    for( auto i = 0; i< 3; i++ )
...
//// DEPRECATED(contact Sergey Korytnik before uncommenting) 
//template< class T >
