...
//#include <LoLevelSupportLib.h>
#include <cassert>
#include <cstdlib>
#include <cstring>
#include <emmintrin.h>

#ifdef PROFILE_DXT_COMPRESSION
extern __int64 g_DTX_Time;
#endif
...
namespace DXTC
{
	...
	void ExtractBlock_SSE2(const BYTE* inPtr, int width, BYTE* colorBlock)
	{
		...
		_mm_store_si128((__m128i*)colorBlock, _mm_loadu_si128((__m128i*)inPtr));
		inPtr += stride;

		// Copy the second row of pixels from inPtr into colorBlock.
		_mm_store_si128((__m128i*)(colorBlock + 16), _mm_loadu_si128((__m128i*)inPtr));
		inPtr += stride;

		// Copy the third row of pixels from inPtr into colorBlock.
		_mm_store_si128((__m128i*)(colorBlock + 32), _mm_loadu_si128((__m128i*)inPtr));
		inPtr += stride;

		// Copy the forth row of pixels from inPtr into colorBlock.
		_mm_store_si128((__m128i*)(colorBlock + 48), _mm_loadu_si128((__m128i*)inPtr));
	}
